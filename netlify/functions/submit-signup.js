...
//
// This keeps the GitHub token safe on the server side — it never
// appears in the browser's page source, unlike embedding it directly
// in the HTML (which GitHub's secret scanner correctly flagged).
//
...
exports.handler = async function (event) {
  ...
  // Basic validation — same checks the frontend already does, repeated
  // here because a server-side function should never fully trust data
  // coming from a browser, even your own form.
  ...
  // Generate the client ID server-side too, so it's never something
  // the browser could tamper with or collide on accidentally.
  ...
  try {
    ...
    if (response.status !== 204) {
      ...
      return {
        statusCode: 502,
        body: JSON.stringify({ error: "Failed to register with GitHub" }),
      };
    }

    // Success — return the generated client ID so the browser can show
    // it to the user, without ever having touched the actual token.
    return {
      statusCode: 200,
